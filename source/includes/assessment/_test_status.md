...
After a successful `send_test` request, Greenhouse will check whether the test instance has been completed by polling the `test_status` endpoint once per hour. We will discontinue polling the `test_status` endpoint after we receive a `partner_status` of `complete`, or after 8 weeks has passed since the test was sent.
